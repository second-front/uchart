...
{{- define "2f.uchart.lib.container.validate" -}}
  ...
  {{- if not (kindIs "map" $containerObject.image)  -}}
    {{- fail (printf "Image required to be a dictionary with repository and tag fields. (workload %s, container %s)" $workloadObject.id $containerObject.id) }}
  {{- end -}}

  {{- if empty (dig "image" "repository" nil $containerObject) -}}
    {{- fail (printf "No image repository specified for container. (workload %s, container %s)" $workloadObject.id $containerObject.id) }}
  {{- end -}}
