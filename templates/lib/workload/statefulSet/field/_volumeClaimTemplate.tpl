...
{{- define "2f.uchart.lib.workload.statefulSet.field.volumeClaimTemplates" -}}
  {{- $root := .root -}}
  ...
  {{- range $id, $volumeClaimTemplate := (dig "statefulSet" "volumeClaimTemplates" dict $statefulSetObject) -}}
    {{- $vct := include "2f.uchart.lib.workload.statefulSet.field.volumeClaimTemplate" (dict "root" $root "id" $id "values" $volumeClaimTemplate) -}}
...
{{- define "2f.uchart.lib.workload.statefulSet.field.volumeClaimTemplate" -}}
  {{- $root := .root -}}
  {{- $id := .id -}}
  {{- $values := .values -}}
