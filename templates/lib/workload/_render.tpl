...
{{- define "2f.uchart.lib.workload.render" -}}
  {{- $root := $ -}}
  ...
  {{- range $key, $workload := $enabledWorkloads -}}
    ...
    {{- if has $workloadObject.type $workloadTypes -}}
      ...
      {{- $resourceObject := include (printf "2f.uchart.lib.workload.%s.initialize" $workloadObject.type) $args | fromYaml -}}
      {{- include (printf "2f.uchart.lib.workload.%s.validate" $workloadObject.type) (dict "root" $root "object" $resourceObject) -}}
      {{- include (printf "2f.uchart.lib.workload.%s.blueprint" $workloadObject.type) (dict "root" $root "object" $resourceObject) | nindent 0 -}}
    {{- end -}}
    ...
    {{- if has $workloadValues.type $scalableWorkloadTypes -}}
      {{- if dig "autoscaling" "enabled" false $workloadObject -}}
        ...
        {{- include "2f.uchart.lib.horizontalPodAutoscaler.blueprint" (dict "root" $root "object" $hpaObject) | nindent 0 -}}
      {{- end -}}
      ...
      {{- if dig "pdb" "enabled" false $workloadObject -}}
        ...
        {{- include "2f.uchart.lib.podDisruptionBudget.blueprint" (dict "root" $root "object" $pdbObject) | nindent 0 -}}
      {{- end -}}
    {{- end -}}
  {{- end -}}
{{- end -}}
